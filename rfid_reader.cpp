...
/* Contains modifications from the original sample application. */
...
void signalHandler(int sig) {
  cout << "\nSignal " << sig << " received. Exiting application.\n";
...
int CMyApplication::run(char *pReaderHostName) {
  ...
  fifo_fd = open(readFifo, O_WRONLY);
...
llrp_u1v_t CMyApplication::hexStrToBitArray(string hex, int lenBits) {
  int i, temp, ptr;
  string cleanHex = "";

  // Remove leading or trialing spaces.
  for (i = 0; i < hex.length(); i++) {
    if (hex[i] != ' ') cleanHex += hex[i];
  }
...
int CMyApplication::enableGpi(int port) {
  ...
  if (checkLLRPStatus(pRsp->getLLRPStatus(), "enableGpi") != 0) {
    delete pRspMsg;
    return -1;
  }
...
int CMyApplication::addROSpec(void) {
  ...
  */
  ...
  for (i = 1; i <= NUM_ANTENNAS; i++) {
    ...
    pAntennaConfig->addAirProtocolInventoryCommandSettings(
  ...
  CImpinjTagReportContentSelector *pImpContentSelector =
      new CImpinjTagReportContentSelector();
...
void CMyApplication::printTagReportData(CRO_ACCESS_REPORT *pRO_ACCESS_REPORT) {
   ...
   */
  for (Cur = pRO_ACCESS_REPORT->beginTagReportData();
       Cur != pRO_ACCESS_REPORT->endTagReportData(); Cur++) {
    nEntry++;
  }
 ...
 **
 *****************************************************************************/
void CMyApplication::formatOneEPC(CParameter *pEPCParameter, char *buf,
                                  int buflen) {
  char *p = buf;
  int bufsize = buflen;
  int written = 0;
 ...
 **
 *****************************************************************************/
void CMyApplication::printOneTagReportData(CTagReportData *pTagReportData) {
  char aBuf[128];
  ...
  // for (Result = pTagReportData->beginAccessCommandOpSpecResult();
  //      Result != pTagReportData->endAccessCommandOpSpecResult(); Result++) {
  ...
  // for (Result = pTagReportData->beginCustom();
  //      Result != pTagReportData->endCustom(); Result++) {
  ...
  std::string output = epc + "," + NumberToString(timestamp) + "," +
                       NumberToString((int)peakRssi) + "," +
                       NumberToString((int)antennaId) + "\n";
  // std::string output = epc + "," + tid + "," + fastId + "," +
  // NumberToString(timestamp) + "," + NumberToString((int) peakRssi) + "\n";
  // cout << output << endl;
  write(fifo_fd, output.c_str(), output.length() + 1);
}

string CMyApplication::formatOneReadResult(CParameter *pOpSpecReadResult) {
  char buffer[256];
  char *p = buffer;
...
void CMyApplication::handleAntennaEvent(CAntennaEvent *pAntennaEvent) {
...
int CMyApplication::checkLLRPStatus(CLLRPStatus *pLLRPStatus, char *pWhatStr) {
  /*
...
CMessage *CMyApplication::transact(CMessage *pSendMsg) {
...
CMessage *CMyApplication::recvMessage(int nMaxMS) {
  CConnection *pConn = m_pConnectionToReader;
  CMessage *pMessage;
...
int CMyApplication::sendMessage(CMessage *pSendMsg) {
  CConnection *pConn = m_pConnectionToReader;
...
void CMyApplication::printXMLMessage(CMessage *pMessage) {
  char aBuf[100 * 1024];
